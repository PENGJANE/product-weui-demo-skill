...
**Stage 1 必须按顺序调用以下三个 skill，不可跳过：**

### 1-A  调用 `brainstorming` skill
...
### 1-B  调用 `ai-work-booster` skill

brainstorm 结束后，对产出的产品逻辑做架构诊断：

- 检验状态流转是否完整（有无遗漏的边界状态）
- 识别硬依赖（API / 权限 / 数据源）是否已全部标注
- 检查逻辑自洽性：条件互斥 / 覆盖全集 / 无死路
- 输出诊断结论，有问题则回到 1-A 补全，无问题继续

### 1-C  调用 `writing-plans` skill（Planning with Files）

用 writing-plans 将产品逻辑落成结构化实施计划文件：

- 把每个功能/状态逐一记录为带优先级的任务条目
- 明确 P0/P1 分级（**必须在 Stage 1 定下来**，不能推迟到 Stage 3）
- 列出每个阶段的验收标准（acceptance criteria）
- 计划文件作为后续各阶段的输入依据
...
**产出物**：一份文字版产品逻辑确认单 + writing-plans 计划文件（含每项功能的 P0/P1 标注）。
**继续条件**：用户确认逻辑无误，所有条目都有优先级，ai-work-booster 诊断无重大遗漏。
